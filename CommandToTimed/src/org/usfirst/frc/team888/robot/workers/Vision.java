...
public class Vision {
    ...
    protected byte[] byteCameraMessage = ByteBuffer.allocate(4).putInt(cycle)
            .array();
    ...
    private Vision() {
        try {
            ...
            message = new DatagramPacket(byteCameraMessage,
                    byteCameraMessage.length, jetsonAddress,
                    RobotMap.JETSON_UDP_PORT);
        } catch (Exception e) {
        }
    }
     ...
     * 
     * @param cycle
     *            The cycle the message was sent
     */
