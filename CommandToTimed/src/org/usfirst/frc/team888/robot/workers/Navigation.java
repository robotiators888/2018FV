...
public class Navigation {
    ...
    // Instantiates a chooser for the dashboard to select where the robot is at
    ...
    // Instantiates boolean for if manual controls are enabled. Defaults to
    // disables (auto).
    ...
    public Navigation() {
        // Sets objects of necessary classes to be the objects passed in by
        // Robot
    ...
    public void navigationExecute() {
        ...
        // Send the location data to the dashboard once per second on the half
        // second
        else if (schedulerOffset == 25) {
            location.updateDashboard();
        }

        // Increment the offset by one. If it has reached 50 (one second), set
        // it back to zero.
    ...
    public void updateMotion() {
        ...
        // Otherwise run teleop controls.
            ...
            // Otherwise go at 70% speed
                ...
                leftBaseDriveOutput = 0.7
                        * oi.getLeftStickAxis(RobotMap.L_Y_AXIS);
                rightBaseDriveOutput = 0.7
                        * oi.getRightStickAxis(RobotMap.R_Y_AXIS);
            }
            // If the joystick is less then 20% in either direction then ignore
            // it
            if (Math.abs(oi.getLeftStickAxis(RobotMap.L_Y_AXIS)) < 0.2 && Math
                    .abs(oi.getRightStickAxis(RobotMap.R_Y_AXIS)) < 0.2) {
    ...
    public void autoRun() {
        ...
        case "Middle":
            ...
            switch (state) {
            case 0:
                if (guidance.goToWaypoint(0.0001, 999999, location.getHeading(),
                        RobotMap.DEFAULT_AUTO_SPEED))
                    state = 10;
                /*
                 * pincer.setPincerPosition(1700, true, 0.0); if
                 * (gameData.charAt(0) == 'L') { // If the robot has not arrived
                 * at the switch... if (gps.goToWaypoint(-75, 80, 0,
                 * RobotMap.DEFAULT_AUTO_SPEED)) { state = 1; } } else { if
                 * (gps.goToWaypoint(72, 80, 0, RobotMap.DEFAULT_AUTO_SPEED)) {
                 * state = 1; } }
                 */
                break;
            case 1:
                ...
                if (gameData.charAt(0) == 'L') {
                    ...
                    if (guidance.goToWaypoint(-75, 96, 0,
                            RobotMap.DEFAULT_AUTO_SPEED)) {
                        state = 2;
                    }
                } else {
                    if (guidance.goToWaypoint(72, 96, 0,
                            RobotMap.DEFAULT_AUTO_SPEED)) {
                        state = 2;
                    }
                }
            ...
            case 3:
                ...
                if (guidance.goToWaypoint(-5.5, 6, 0,
                        -RobotMap.DEFAULT_AUTO_SPEED)) {
                    state = 4;
                }
                break;
            case 4:
                ...
                if (cubeLocation == null) {
                    if (guidance.goToWaypoint(-5.5, 54, 0,
                            RobotMap.DEFAULT_AUTO_SPEED)) {
                        state = 5;
                    }
                } else {
            ...
            case 6:
                pincer.setPincerPosition(1100, true, 0.0);
                if (gameData.charAt(0) == 'L') {
                    if (guidance.goToWaypoint(-30, 30, ((11 * Math.PI) / 6),
                            -RobotMap.DEFAULT_AUTO_SPEED)) {
                        state = 7;
                    }
                } else {
                    if (guidance.goToWaypoint(30, 30, (Math.PI / 6),
                            -RobotMap.DEFAULT_AUTO_SPEED)) {
                        state = 7;
                    }
                }
            ...
            case 7:
                ...
                pincer.setPincerPosition(1100, true, 0.0);
                if (gameData.charAt(0) == 'L') {
                    if (guidance.goToWaypoint(-75, 80, 0,
                            RobotMap.DEFAULT_AUTO_SPEED)) {
                        state = 8;
                    }
                } else {
                    if (guidance.goToWaypoint(72, 80, 0,
                            RobotMap.DEFAULT_AUTO_SPEED)) {
                        state = 8;
                    }
                }
            ...
            case 8:
                ...
                if (gameData.charAt(0) == 'L') {
                    // If the robot has not arrived at the switch...
                    if (guidance.goToWaypoint(-75, 93, 0,
                            RobotMap.DEFAULT_AUTO_SPEED)) {
                        state = 9;
                    }
                } else {
                    if (guidance.goToWaypoint(72, 93, 0,
                            RobotMap.DEFAULT_AUTO_SPEED)) {
                        state = 9;
                    }
                }
        ...
        case "Left":
            ...
            case 0:
                ...
                if (gameData.charAt(0) == 'L') {
                    if (guidance.goToWaypoint(0, 148, (Math.PI / 2),
                            RobotMap.DEFAULT_AUTO_SPEED)) {
                        state = 1;
                    }
                } else {
                    if (guidance.goToWaypoint(0, 210, (Math.PI / 2),
                            RobotMap.DEFAULT_AUTO_SPEED)) {
            ...
            case 1:
                pincer.setPincerPosition(1800, true, 0.0);
                if (gameData.charAt(0) == 'L') {
                    if (guidance.goToWaypoint(20, 148, (Math.PI / 2),
                            RobotMap.DEFAULT_AUTO_SPEED)) {
                        state = 4;
                    }
                } else {
                    if (guidance.goToWaypoint(226, 200, Math.PI,
                            RobotMap.DEFAULT_AUTO_SPEED)) {
                        state = 2;
                    }
                }
            ...
            case 2:
                ...
                if (guidance.goToWaypoint(226, 160, ((Math.PI * 3) / 2),
                        RobotMap.DEFAULT_AUTO_SPEED)) {
                    state = 3;
                }
                break;
            case 3:
                pincer.setPincerPosition(1500, true, 0.0);
                if (guidance.goToWaypoint(206, 160, ((Math.PI * 3) / 2),
                        RobotMap.DEFAULT_AUTO_SPEED)) {
                    state = 4;
                }
                break;
        ...
        case "Right":
            ...
            case 0:
                ...
                if (gameData.charAt(0) == 'R') {
                    if (guidance.goToWaypoint(0, 148, ((Math.PI * 3) / 2),
                            RobotMap.DEFAULT_AUTO_SPEED)) {
                        state = 1;
                    }
                } else {
                    if (guidance.goToWaypoint(0, 210, ((Math.PI * 3) / 2),
                            RobotMap.DEFAULT_AUTO_SPEED)) {
                        if (strategy.getSelected().equals("Cancel")) {
                            state = 6;
                        } else {
            ...
            case 1:
                pincer.setPincerPosition(1800, true, 0.0);
                if (gameData.charAt(0) == 'R') {
                    if (guidance.goToWaypoint(-20, 148, ((Math.PI * 3) / 2),
                            RobotMap.DEFAULT_AUTO_SPEED)) {
                        state = 4;
                    }
                } else {
            ...
            case 2:
                pincer.setPincerPosition(1500, true, 0.0);
                if (guidance.goToWaypoint(-226, 160, (Math.PI / 2),
                        RobotMap.DEFAULT_AUTO_SPEED)) {
                    state = 3;
                }
                break;
            case 3:
                pincer.setPincerPosition(1500, true, 0.0);
                if (guidance.goToWaypoint(-206, 160, (Math.PI / 2),
                        RobotMap.DEFAULT_AUTO_SPEED)) {
                    state = 4;
                }
                break;
