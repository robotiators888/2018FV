	...
	public void autoRun() {
		...
		case "Middle":	
			...
			case 0:
				...
				if (gameData.charAt(0) == 'L') {
					...
					if (!gps.goToWaypoint(0, 36, ((Math.PI * 7) / 4))) {
						// ...go there.
						gps.goToWaypoint(0, 36, ((Math.PI * 7) / 4));
					}
					// Otherwise...
					else {
						// Otherwise...
					...
					// If the robot has not arrived at the switch...
					if (!gps.goToWaypoint(0, 36, ((Math.PI) / 4))) {
						// ...go there.
						gps.goToWaypoint(0, 36, ((Math.PI) / 4));
					}
					// Otherwise...
					else {
						// Otherwise...
			...
			case 1:
				...
				if (gameData.charAt(0) == 'L') {
					// If the robot has not arrived at the switch...
					if (!gps.goToWaypoint(-75, 89.5, 0)) {
						// ...go there.
						gps.goToWaypoint(-75, 89.5, 0);
					}
					// Otherwise...
					else {
						// Otherwise...
				...
				// If the our alliance has the right side of the switch
					...
					// If the robot has not arrived at the switch...
					if (!gps.goToWaypoint(72, 89.5, 0)) {
						// ...go there.
						gps.goToWaypoint(72, 89.5, 0);
					}
					// Otherwise...
					else {
						// Otherwise...
		...
		case "Left":
			...
			case 1: 
				pincer.setPincerPosition(1800, true, 0.0);
				if (gameData.charAt(0) == 'L') {
					if (!gps.goToWaypoint(20, 148, (Math.PI / 2))) {
						gps.goToWaypoint(20, 148, (Math.PI / 2));
					}
					else {
						state = 3;
					}
					...
					if (!gps.goToWaypoint(154, 200, Math.PI)) {
						gps.goToWaypoint(154, 200, Math.PI);
					}
					else {
						state = 2;
					}
			...
			case 2:
				pincer.setPincerPosition(1800, true, 0.0);
				if (!gps.goToWaypoint(154, 185, Math.PI)) {
					gps.goToWaypoint(154, 185, Math.PI);
				}
				else {
					state = 3;
				}
		...
		case "Right":
			...
			case 1: 
				pincer.setPincerPosition(1800, true, 0.0);
				if (gameData.charAt(0) == 'R') {
					if (!gps.goToWaypoint(-20, 148, ((Math.PI * 3) /2))) {
						gps.goToWaypoint(-20, 148, ((Math.PI * 3) /2));
					}
					else {
						state = 3;
					}
