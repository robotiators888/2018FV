...
public class Navigation extends Subsystem {
	...
	public Navigation(DriveTrain p_drive, DeadReckon p_location, Pincer p_pince, OI p_oi) {
		...
		startPosition.addDefault("Don't Drop Cube", "Middle");
	...
	public void autoRun() {
		//gameData.charAt(0)
		//startPosition.getSelected().charAt(0)
		...
		case 1: {
			if(gameData.charAt(0) == 'L'){
				drive.move(-RobotMap.LEFT_AUTO_SPEED, RobotMap.RIGHT_AUTO_SPEED);
				if(Math.abs(location.getHeading()) > 0.78){
					state = 2;
				}
				else{
					state = 1;
		...
		case 3: {
			if(gameData.charAt(0) == 'L'){
				drive.move(RobotMap.LEFT_AUTO_SPEED, -RobotMap.RIGHT_AUTO_SPEED);
				if(Math.abs(location.getHeading()) < 0.05){
					state = 4;
				}
				else{
					state = 3;
